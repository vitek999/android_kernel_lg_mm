...
static unsigned int g_limited_freq_by_hevc;
...
static unsigned int g_limited_min_freq_by_kdriver;
static unsigned int g_limited_max_freq_by_kdriver;
...
static unsigned int mt_cpufreq_limit_max_freq_by_early_suspend(void)
{
    ...
    cpufreq_info("mt_cpufreq limited max freq by early suspend %d\n", DVFS_F2);
...
static unsigned int mt_cpufreq_volt_to_pmic_wrap(unsigned int target_volt)
{
    ...
    cpufreq_dbg("mt_cpufreq_volt_to_pmic_wrap: current pmic wrap idx = %d\n", idx);
    return idx;
}
#endif
...
unsigned int mt_cpufreq_voltage_set_by_ptpod(unsigned int pmic_volt[], unsigned int array_size)
{
    ...
    {
        cpufreq_warn("mt_cpufreq_voltage_set_by_ptpod: ERROR!array_size is invalide, array_size = %d\n", array_size);
    }
    ...
    for (i = 0; i < array_size; i++)
    {
        ...
        cpufreq_dbg("mt_cpufreq_pmic_volt[%d] = %x\n", i, mt_cpufreq_pmic_volt[i]);
    }
    ...
    cpufreq_dbg("mt_cpufreq_voltage_set_by_ptpod: Set voltage directly by PTP-OD request! mt_cpufreq_ptpod_voltage_down = %d\n", mt_cpufreq_ptpod_voltage_down);
...
unsigned int mt_cpufreq_max_frequency_by_DVS(unsigned int num)
{
    ...
    {
        cpufreq_info("PTPOD0:num = %d, frequency= %d\n", num, mt_cpu_freqs[0].cpufreq_khz);
    ...
    for (i = 1; i < mt_cpu_freqs_num; i++)
    {
        ...
        if(num == voltage_change_num)
        {
            cpufreq_info("PTPOD1:num = %d, frequency= %d\n", num, mt_cpu_freqs[i].cpufreq_khz);
			return mt_cpu_freqs[i].cpufreq_khz;
        }
    }

    cpufreq_info("PTPOD2:num = %d, NOT found! return 0!\n", num);
    return 0;
}
...
static void mt_cpufreq_power_calculation(int index, int ncpu)
{
    ...
    cpufreq_info("mt_cpu_power[%d]: cpufreq_ncpu = %d, cpufreq_khz = %d, cpufreq_power = %d\n", (index * possiblecpu + ncpu),
...
void mt_cpufreq_volt_set_by_sdio(unsigned int volt)
{
	...
	{
		cpufreq_warn("mt_cpufreq_volt_set_by_sdio, could not set volt now,  mt_cpufreq_disabled_by_sdio_autoK = %d\n", mt_cpufreq_disabled_by_sdio_autoK);
	}
#endif
}
        ...
        switch (target_volt)
        {
            ...
            case DVFS_V2:
				...
				if(g_cpufreq_get_vcore_corner == false)
					...
					cpufreq_dbg("switch to DVS2_0: %d mV\n", DVFS_V2_0);
				#else
                cpufreq_dbg("switch to DVS2: %d mV\n", DVFS_V2);
				#endif
            ...
            #ifdef MT_DVFS_LOW_VOLTAGE_SUPPORT
            case DVFS_V3:
                cpufreq_dbg("switch to DVS3: %d mV\n", DVFS_V3);
...
static int mt_cpufreq_target(struct cpufreq_policy *policy, unsigned int target_freq, unsigned int relation)
{
    ...
    *************************************************/
	...
	/**************************************
	* Search for min freq of kdriver needed
	***************************************/
	if ((g_limited_min_freq_by_kdriver != 0) && (freqs.new < g_limited_min_freq_by_kdriver)) {
		for (i = (mt_cpu_freqs_num - 1); i >= 0; i--) {
			if (mt_cpu_freqs[i].cpufreq_khz >= g_limited_min_freq_by_kdriver) {
				freqs.new = mt_cpu_freqs[i].cpufreq_khz;
				cpufreq_dbg("kdriver limit min freq! freqs.new = %d\n", freqs.new);
				break;
			}
		}
	}
    ...
    ***************************************/
	if((g_limited_freq_by_hevc != 0) && (freqs.new < g_limited_freq_by_hevc)) {
		for (i = (mt_cpu_freqs_num - 1); i >= 0; i--) {
			if (mt_cpu_freqs[i].cpufreq_khz >= g_limited_freq_by_hevc) {
				freqs.new = mt_cpu_freqs[i].cpufreq_khz;
				cpufreq_dbg("HEVC limit freq! freqs.new = %d\n", freqs.new);
				break;
			}
		}
	}
    ...
    *************************************************/
	...
	/*****************************************
	* Search for max freq of kdriver requested
	******************************************/
	if ((g_limited_max_freq_by_kdriver != 0) && (freqs.new > g_limited_max_freq_by_kdriver)) {
		for (i = 0; i < mt_cpu_freqs_num; i++) {
			if (mt_cpu_freqs[i].cpufreq_khz <= g_limited_max_freq_by_kdriver) {
				freqs.new = mt_cpu_freqs[i].cpufreq_khz;
				cpufreq_dbg("kdriver limit max freq! freqs.new = %d\n", freqs.new);
				break;
			}
		}
	}
    ...
    *************************************************/
    ...
    if((num_online_cpus() < num_possible_cpus()) && (freqs.new > DVFS_F2))
    {
        ...
        {
            cpufreq_dbg("Limited frequency, because num_online_cpus() = %d, freqs.new = %d\n", num_online_cpus(), freqs.new);
            freqs.new = DVFS_F2;
        }
    }
    #endif
...
void mt_cpufreq_thermal_protect(unsigned int limited_power)
{
    ...
    if (limited_power == 0)
    {
        ...
        cpufreq_info("thermal limit g_limited_max_freq = %d, g_limited_max_ncpu = %d\n", g_limited_max_freq, g_limited_max_ncpu);
    }
    else
    {
        while (ncpu)
        ...
        if (!found)
        {
            cpufreq_warn("Not found suitable DVFS OPP, limit to lowest OPP!\n");
        ...
        if (num_online_cpus() > g_limited_max_ncpu)
        {
            ...
            {
                cpufreq_info("turn off CPU%d due to thermal protection\n", (i - 1));
                cpu_down((i - 1));
            }
        }
...
void mt_cpufreq_set_min_freq(unsigned int id, unsigned int freq)
{
	struct cpufreq_policy *policy;

	g_limited_min_freq_by_kdriver = freq;

	cpufreq_info("limited_min_freq_by_kdriver! g_limited_min_freq_by_kdriver = %d\n",
		g_limited_min_freq_by_kdriver);

	policy = cpufreq_cpu_get(0);
	if (policy) {
		if (policy->cur < g_limited_min_freq_by_kdriver)
			mt_cpufreq_target(policy, g_limited_min_freq_by_kdriver, CPUFREQ_RELATION_L);
		cpufreq_cpu_put(policy);
	}
}

void mt_cpufreq_set_max_freq(unsigned int id, unsigned int freq)
{
	struct cpufreq_policy *policy;

	g_limited_max_freq_by_kdriver = freq;

	cpufreq_info("limited_max_freq_by_kdriver! g_limited_max_freq_by_kdriver = %d\n",
		g_limited_max_freq_by_kdriver);

	policy = cpufreq_cpu_get(0);
	if (policy) {
		if (g_limited_max_freq_by_kdriver != 0 && policy->cur > g_limited_max_freq_by_kdriver)
			mt_cpufreq_target(policy, g_limited_max_freq_by_kdriver, CPUFREQ_RELATION_H);
		cpufreq_cpu_put(policy);
	}
}
...
static ssize_t mt_cpufreq_limited_load_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    {
        cpufreq_err("bad argument!! please provide the limited load\n");
    }
...
static int mt_cpufreq_limited_by_hevc_read(struct seq_file *m, void *v)
{
    seq_printf(m, "g_limited_freq_by_hevc = %d\n", g_limited_freq_by_hevc);

    return 0;
}

static ssize_t mt_cpufreq_limited_by_hevc_write(struct file *file,
				const char *buffer, unsigned long count, void *data)
{
	...
	if (sscanf(buffer, "%u", &freq) == 1) {
		g_limited_freq_by_hevc = freq;
		cpufreq_info("limited_by_hevc! g_limited_freq_by_hevc = %d\n", g_limited_freq_by_hevc);

		policy = cpufreq_cpu_get(0);
		if (policy) {
			if (policy->cur < g_limited_freq_by_hevc)
				cpufreq_driver_target(policy, g_limited_freq_by_hevc, CPUFREQ_RELATION_L);
			cpufreq_cpu_put(policy);
		}

		return count;
	} else
        cpufreq_err("bad argument!! please provide the min limit\n");
...
static int proc_mt_cpufreq_limited_by_hevc_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_limited_by_hevc_read, NULL);
}
static const struct file_operations cpufreq_limited_by_hevc_proc_fops = {
    .owner = THIS_MODULE,
    .open  = proc_mt_cpufreq_limited_by_hevc_open,
    .read = seq_read,
    .write = mt_cpufreq_limited_by_hevc_write,
    .release = single_release,
};
...
static int __init mt_cpufreq_pdrv_init(void)
{
    ...
    else
    {
        ...
        if (proc_create("cpufreq_debug", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &cpufreq_debug_proc_fops) == NULL) {
            cpufreq_err("%s: create_proc_entry cpufreq_debug failed\n", __FUNCTION__);
            ASSERT(0);
            return -1;
        }

        if (proc_create("cpufreq_limited_power", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &cpufreq_limited_power_proc_fops) == NULL) {
            cpufreq_err("%s: create_proc_entry cpufreq_limited_power failed\n", __FUNCTION__);
            ASSERT(0);
            return -1;
        }

        if (proc_create("cpufreq_state", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &cpufreq_state_proc_fops) == NULL) {
            cpufreq_err("%s: create_proc_entry cpufreq_state failed\n", __FUNCTION__);
            ASSERT(0);
            return -1;
        }

        if (proc_create("cpufreq_power_dump", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &cpufreq_power_dump_proc_fops) == NULL) {
            cpufreq_err("%s: create_proc_entry cpufreq_power_dump failed\n", __FUNCTION__);
            ASSERT(0);
            return -1;
        }

		if (proc_create("cpufreq_limited_by_hevc", S_IRUGO | S_IWUSR | S_IWGRP,
			mt_cpufreq_dir, &cpufreq_limited_by_hevc_proc_fops) == NULL) {
			cpufreq_err("%s: create_proc_entry cpufreq_limited_by_hevc failed\n", __FUNCTION__);
			ASSERT(0);
			return -1;
		}
        ...
        if (proc_create("cpufreq_limited_load", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &cpufreq_limited_load_proc_fops) == NULL) {
            cpufreq_err("%s: create_proc_entry cpufreq_limited_load failed\n", __FUNCTION__);
            ASSERT(0);
            return -1;
        }
        #endif
        ...
        if (proc_create("cpufreq_ptpod_test", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &cpufreq_ptpod_test_proc_fops) == NULL) {
            cpufreq_err("%s: create_proc_entry cpufreq_ptpod_test failed\n", __FUNCTION__);
            ASSERT(0);
            return -1;
        }
        #endif
