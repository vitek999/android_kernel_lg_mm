...
static int iReadData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char *pinputdata)
{
	...
	/* CAM_CALDB("[S24EEPORM] iReadData finial address is %d length is
	 %d buffer address is 0x%x\n",u4CurrentOffset, i4ResidueDataLength, pBuff); */
...
/*	if (_IOC_NONE == _IOC_DIR(a_u4Command)) {
	...
	if (_IOC_NONE != _IOC_DIR(a_u4Command)) {
		...
		//LGE_UPDATE_S [2016/03/27] youmi.jun@lge.com, static analysis
		ptempbuf = (stCAM_CAL_INFO_STRUCT *)pBuff;
		pWorkingBuff = kmalloc(ptempbuf->u4Length, GFP_KERNEL);
		//LGE_UPDATE_E [2016/03/27] youmi.jun@lge.com, static analysis
	}
