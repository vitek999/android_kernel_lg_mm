...
void HI191MIPI_SetGain(UINT16 iGain)
{
	...
	//return iGain; //kipo.yun WBT void function returns value
	#endif
}	/*	HI191MIPI_SetGain	*/
...
static kal_uint32 HI191MIPIset_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    kal_uint32 pclk;
    kal_int16 dummyLine;
    kal_uint16 lineLength,frameHeight;

    SENSORDB("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	//max_exposure_lines = HI191MIPI_sensor.pclk * HI191MIPI_FPS(1) / (pv_min_fps * HI191MIPI_sensor.line_length);

    switch (scenario_id) {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
            pclk =  HI191MIPI_sensor.pclk;
            lineLength = HI191MIPI_PV_PERIOD_PIXEL_NUMS + HI191MIPI_DEFAULT_DUMMY_PIXELS;
            frameHeight = (10 * pclk)/framerate/lineLength;
            dummyLine = frameHeight - (HI191MIPI_PV_PERIOD_LINE_NUMS + HI191MIPI_DEFAULT_DUMMY_LINES);
            if(dummyLine<0)
                dummyLine = 0;
            spin_lock(&hi191mipiraw_drv_lock);
            HI191MIPI_sensor.pv_mode = KAL_TRUE;
            spin_unlock(&hi191mipiraw_drv_lock);
            HI191MIPI_Set_Dummy(0, dummyLine);
            break;
        case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
            pclk =  HI191MIPI_sensor.pclk;
            lineLength = HI191MIPI_PV_PERIOD_PIXEL_NUMS + HI191MIPI_DEFAULT_DUMMY_PIXELS;
...
static kal_uint32 HI191MIPIget_default_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 *framerate)
{
							 ...
							 UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
	...
	switch (FeatureId)
	{
        ...
        case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
            HI191MIPIset_max_framerate_by_scenario((MSDK_SCENARIO_ID_ENUM)*pFeatureData32, *(pFeatureData32+1));
            break;
        case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
            HI191MIPIget_default_framerate_by_scenario((MSDK_SCENARIO_ID_ENUM)*pFeatureData32, (MUINT32 *)(*(pFeatureData32+1)));
            break;
