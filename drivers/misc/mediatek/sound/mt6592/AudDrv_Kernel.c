...
#ifdef CONFIG_MTK_AUDIO_EXTCODEC_SUPPORT
...
void Auddrv_Handle_Mem_context(AFE_MEM_CONTROL_T *Mem_Block)
{
	...
	if (HW_Cur_ReadIdx < mBlock->pucPhysBufAddr ||
	(HW_Cur_ReadIdx >= (mBlock->u4BufferSize + mBlock->pucPhysBufAddr))) {
		PRINTK_AUDDRV("[Auddrv] Read HW_Cur_ReadIdx ==[%d]\n", HW_Cur_ReadIdx);
		return;
	}
        ...
        {
            {
                ...
                PRINTK_AUDDRV("SET_GPIO_CURRENT GPIO_normal after setting = %x, arg= %lu,\n",value,arg);
...
// 2014-06-27 hyeonsang85.park@lge.com : let tpa2028d know current device for combo_path gain control - end
	...
	pr_warn("Afe_Mem_Pwr_on =0x%x\n", Afe_Mem_Pwr_on);
	pr_warn("Aud_AFE_Clk_cntr = 0x%x\n", Aud_AFE_Clk_cntr);
	pr_warn("Aud_ANA_Clk_cntr = 0x%x\n", Aud_ANA_Clk_cntr);
	pr_warn("Aud_HDMI_Clk_cntr = 0x%x\n", Aud_HDMI_Clk_cntr);
	pr_warn("Aud_I2S_Clk_cntr = 0x%x\n", Aud_I2S_Clk_cntr);
	pr_warn("Aud_LJ22M_Clk_cntr = 0x%x\n", Aud_LJ22M_Clk_cntr);
	pr_warn("Aud_LJ24M_Clk_cntr = 0x%x\n", Aud_LJ24M_Clk_cntr);
	pr_warn("AuddrvSpkStatus = 0x%x\n", AuddrvSpkStatus);
            Ana_Log_Print();
            Afe_Log_Print();
            break;
        }
        ...
        {
            pr_debug("+AUDDRV_GET_IPO_EVER AudDrvSuspend_ipoh_Status = %d\n ", AudDrvSuspend_ipoh_Status);
            ret = AudDrvSuspend_ipoh_Status;
            break;
        }
        case AUDDRV_CLEAR_IPO_EVER:
        {
            pr_debug("+AUDDRV_CLEAR_IPO_EVER\n");
